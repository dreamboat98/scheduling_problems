...
\includegraphics[trim = 2cm 8cm 2cm 9cm, clip, width = .5\textwidth]{./figures/V_max_schedules_no_effect_day.pdf}
}
\subfloat[][]{
\label{fig:V_max_voltages_no_effect_day}
\includegraphics[trim = 2cm 8cm 2cm 9cm, clip, width = .5\textwidth]{./figures/V_max_voltages_no_effect_day.pdf}
}
...
An important aspect of RFB economics is the variability of daily price profile across the year. Figure XX shows composite day-ahead price data for the winter and summer seasons in 2017. In summer months, the availability of cheap solar power depresses the price around midday, and the peak evening demand is low, resulting in two peaks of similar height. In winter, the solar price depression is not so marked, and the price in the evening peak becomes higher, hence the profile is more like a single peak.

\cref{fig:winter_day_schedules,fig:summer_day_schedules} show the optimal LP and NLP (\cref{Model_Formulation_Constraint_Cell_Voltage}) schedules for example winter and summer days from 2017. Although the revenue obtained is \pounds 0.074 for both days under the NLP optimal schedule, the required throughput of the battery is greater on the summer day. This has an impact on the error of the LP revenue estimation - for the winter day the revenue is estimated at \pound 0.088, whereas on the summmer day it is estimated at \pound 0.099. The error in the estimation of revenue under the LP optimisation is dependent on energy throughput.
