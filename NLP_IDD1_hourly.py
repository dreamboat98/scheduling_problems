# coding: utf-8
"""NLP schedule optimisation using IDD1 parameters"""
########################################################################################################################
...
def objective_expression(model):
    return model.StackArea * model.TimeStep * (1 / 1000000) \
            * sum(model.PriceBM[t] *
                  ((model.VOCV50SOC - model.Vfaradaic) * model.CurrentDensityDischarge[t] * (1-LossBOP)\
                  - (model.VOCV50SOC + model.Vfaradaic) * model.CurrentDensityCharge[t] * (1/(1-LossBOP)) \
...
results_table.to_csv('CR_NLP_IDD1_results.csv', sep=',')
